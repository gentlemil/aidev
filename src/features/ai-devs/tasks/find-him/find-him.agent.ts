...
import { findHimToolDefinitions, execute } from './find-him.tools'
import { AIProviders, PROVIDER_API } from '@/lib/ai-models'
import type { FindHimStreamEvent } from './find-him.events'
import { suspects } from './find-him.consts'
...
export async function runFindHimAgent(
  model: string,
  provider: AIProviders,
  onEvent: (event: FindHimStreamEvent) => void
): Promise<void> {
  ...
  for (let i = 0; i < config.maxIterations; i++) {
    ...
    for (const toolCall of message.tool_calls) {
      ...
      // Capture the final answer when the agent submits
  ...
  throw new Error(`Agent exceeded ${config.maxIterations} iterations without finishing`)
}
